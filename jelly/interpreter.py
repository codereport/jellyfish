...
code_page += "ṕṔÉ①ḳḾℙ≤≥Ď" # jellyfish specific
...
def create_chain(chain, arity = -1, is_forward = True):
    return attrdict(
        arity = arity,
        chain = chain,
        call = lambda x = None, y = None: variadic_chain(chain, is_forward and (x, y) or (y, x))
    )
...
def from_factorial_base(digits):
    place_value = 1
    integer = 0
    for next_place_index, digit in enumerate(digits[::-1], 1):
        integer += digit * place_value
        place_value *= next_place_index
    return integer

def from_primorial_base(digits):
    integer = digits and digits[-1] or 0
    for place_index, digit in enumerate(digits[-2::-1], 1):
        integer += digit * sympy.ntheory.generate.primorial(place_index)
    return integer
...
def get_request(url):
    ...
    url = (re.match(r"[A-Za-z][A-Za-z0-9+.-]*://", url) is None and "http://" or "") + url
...
def stringify(iterable, recurse = True):
    ...
    if str in map(type, iterable) and list not in map(type, iterable) or not iterable:
...
def to_primorial_base(integer):
    ...
    while integer:
        place_index += 1
        integer, remainder = divmod(integer, sympy.ntheory.generate.prime(place_index))
        digits.append(remainder)
    return digits[::-1]
...
atoms = {
    ...
    "①": attrdict(
        arity = 1,
        ldepth = 0,
        call = lambda z: functools.reduce(lambda x, y: dyadic_link(atoms["+"], (x, y)), to_base(z, 2), 0)
    ),
    "b": attrdict(
        arity = 2,
        ldepth = 0,
    ...
    "Ď": attrdict(
        arity = 1,
        ldepth = 1,
        call = lambda z: [int(z[i] != z[i - 1]) for i in range(1, len(z))]
    ),
    "İ": attrdict(
        arity = 1,
        ldepth = 0,
    ...
    "ÆP": attrdict(
        arity = 1,
        ldepth = 0,
        call = lambda z: int(sympy.isprime(z))
    ),
    "Æp": attrdict(
        arity = 1,
        ldepth = 0,
...
str_litlist = r"\[*" + str_literal + r"(?:(?:\]*,\[*)" + str_literal + ")*" + r"\]*"
